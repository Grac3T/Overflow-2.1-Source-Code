...
{
    public AimG(final String checkName, final String type, final CheckType checkType, final boolean enabled) {
    ...
    public void onPacket(final PacketEvent e) {
        ...
        if (this.isPacketMovement(e.getType()) && user != null && System.currentTimeMillis() - user.lastAttackPacket < 500L) {
            ...
            if (MathUtil.isValueVrySmall(pitch)) {
                if (user.aimLVerbose < 20) {
                    final User user2 = user;
                    ++user2.aimLVerbose;
                }
            }
            else if (user.aimLVerbose > 0) {
                final User user3 = user;
                --user3.aimLVerbose;
            }
            if (user.aimLVerbose > 7) {
