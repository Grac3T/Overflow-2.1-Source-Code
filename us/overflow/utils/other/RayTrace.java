...
public class RayTrace
{
    ...
    public Vector positionOfIntersection(final BoundingBox boundingBox, final double skip, final double blocksAway, final double accuracy) {
        final List<Vector> positions = this.traverse(skip, blocksAway, accuracy);
        for (final Vector position : positions) {
            if (intersects(position, boundingBox.getMinimum(), boundingBox.getMaximum())) {
                return position;
            }
    ...
    public boolean intersects(final BoundingBox boundingBox, final double skip, final double blocksAway, final double accuracy) {
        final List<Vector> positions = this.traverse(blocksAway, accuracy);
        for (final Vector position : positions) {
            if (intersects(position, boundingBox.getMinimum(), boundingBox.getMaximum())) {
                return true;
